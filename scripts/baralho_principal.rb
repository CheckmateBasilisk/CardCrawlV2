...
input_filenames = ['baralho_principal_PJ_basic_set.xlsx', 'baralho_principal_DM_basic_set.xlsx']
...
input_filenames.each do |filename|
  ...
  Squib::Deck.new cards: data['name'].size, layout: 'card_layout.yml', width: 750, height: 1050 do
    ...
    # FIXME: the large icon doesn't count towards line height and the tag :foo: doesn't count as text, so the flavor text overlaps with the large icons if there's no other text. Still need to study how to solve this in a scalable way. Maybe adding another column to the xlsx?
